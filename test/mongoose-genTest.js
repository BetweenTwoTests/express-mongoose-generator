...
describe('mongoose-gen', function () {
  ...
  it('--help, should print help', function (done) {
    run('', ['--help'], function (err, stdout) {
      ...
      assert.ok(/-m, --model <modelName> {2}model name/.test(stdout));
      assert.ok(/-f, --fields <fields> {4}model fields/.test(stdout));
      assert.ok(/-r, --rest {15}enable generation REST/.test(stdout));
      assert.ok(/-t, --tree <tree> {8}files tree generation grouped by <t>ype or by <m>odule/.test(stdout));
      done();
    });
  });
});
