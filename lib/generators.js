...
function generateModel(path, modelName, modelFields, generateMethod, ts, cb) {
    ...
    model = model.replace(/{modelNameCaptialized}/g, formatTools.capitalizeFirstLetter(modelName))
...
function generateRouter(path, modelName, generateMethod, ts, cb) {
    ...
    if (generateMethod === 't') {
        ft.createDirIfIsNotDefined(path, 'routes', function () {
            router = router.replace(/{controllerPath}/g, '\'../app/controllers/' + modelName + 'Controller' + ((ts) ? '': '.' + extension) + '\'');
...
function generateController(path, modelName, modelFields, generateMethod, ts, cb) {
    ...
    controller = controller.replace(/{modelNameCaptialized}/, formatTools.capitalizeFirstLetter(modelName));
